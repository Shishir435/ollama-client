...
description: App-owned frontend primitives, shadcn ownership boundaries, token policy, and visual QA checklist for the 0.7 frontend foundation.
---
...
- Use `SettingsField` or `SettingsSliderField` when the control needs label, description, value badge, or validation text.
