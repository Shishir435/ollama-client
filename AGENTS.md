...
- `src/features/selection-actions/` reads its view state from `selection-overlay-context.tsx`, not props. `SelectionOverlayApp` builds the value and is the only place that knows about the reducer, the capture, or the content script's refs; `SelectionActionsOverlay`, `SelectionPanel`, `SelectionToolbar`, `PanelHeader`, and `PanelFooter` take no props at all. Add a new control to the context value and read it where it renders. `PanelMarkdown` and `PanelThinking` stay prop-driven — they are leaf presentational components with their own tests.
