...
**Favicons are the tier below that**, for unrecognized *remote* providers only (`provider-favicon.ts`, served by `providers.icons`). The configured base URL is always asked first; its parent site is asked **only** when that host gave a settled "nothing here" (401/403/404/410, or a 200 carrying something that is not an image — an API gateway guards `/favicon.ico` behind its key exactly like every other path). A timeout or a 5xx is never chased, exactly one label is stripped (`api.acme.com` → `acme.com`, never down to a public suffix), and a third-party favicon service is never used, since that would hand every configured provider URL to whoever runs it. Local and private-range hosts are skipped, the response is sniffed from its leading bytes rather than trusted from `Content-Type`, it is capped at 32KB, and both hits and misses are remembered device-local so an endpoint is asked once. Users can turn the lookup off; doing so also drops what was already fetched.
