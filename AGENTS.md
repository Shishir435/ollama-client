...
- **Favicons are the tier below**, for unrecognized *remote* providers only (`provider-favicon.ts`, served by `providers.icons`). Rules, all load-bearing:
  ...
  - The walk goes straight to the registrable domain (`integrate.api.nvidia.com` → `nvidia.com`), never down to a public suffix. `www.` is asked directly because vendor sites redirect their bare domain there, and redirects stay refused.
