...
export const RAGSettings = () => {
  ...
  const [knowledgeMinRerankScore, setKnowledgeMinRerankScore] = useState(
    DEFAULT_EMBEDDING_CONFIG.minRerankScore
  )
  ...
  useEffect(() => {
    ...
    setKnowledgeMinRerankScore(
      activeSet.retrieval?.minRerankScore ??
        DEFAULT_EMBEDDING_CONFIG.minRerankScore
    )
          ...
          <AccordionContent className="space-y-6 pt-4">
            ...
            {activeSet && (
              <>
                ...
                <SettingsFormField
                  label={`${t("knowledge_sets.min_rerank_label")} (${knowledgeMinRerankScore.toFixed(2)})`}
                  description={t("knowledge_sets.min_rerank_description")}>
                  <Slider
                    value={[knowledgeMinRerankScore]}
                    min={0}
                    max={1}
                    step={0.05}
                    onValueChange={([val]) => {
                      setKnowledgeMinRerankScore(val)
                      updateKnowledgeRetrieval({ minRerankScore: val })
                    }}
                  />
                </SettingsFormField>
              </>
