...
describe("backupService", () => {
  ...
  beforeEach(() => {
    ...
    global.chrome.runtime.getManifest = vi.fn().mockReturnValue({ version: "1.0.0" })
  })
  ...
  describe("exportAll", () => {
    it("should export all components into a zip", async () => {
      ...
      expect(zipInstance.file).toHaveBeenCalledWith("manifest.json", expect.any(String))
      expect(zipInstance.file).toHaveBeenCalledWith("sync-storage.json", expect.any(String))
      expect(zipInstance.file).toHaveBeenCalledWith("local-storage.json", expect.any(String))
      expect(zipInstance.file).toHaveBeenCalledWith("database.sqlite", expect.any(Uint8Array))
      expect(zipInstance.file).toHaveBeenCalledWith("chat-db.json", expect.any(Blob))
      expect(zipInstance.file).toHaveBeenCalledWith("vector-db.json", expect.any(Blob))
      expect(zipInstance.file).toHaveBeenCalledWith("knowledge-db.json", expect.any(Blob))
    })
  })
  ...
  describe("importAll", () => {
    it("should fail if manifest is missing", async () => {
      ...
      await expect(backupService.importAll(new File([], "test.zip")))
    ...
    it("should import all components successfully", async () => {
      ...
      const zipInstance = {
        file: vi.fn().mockImplementation((name) => {
          if (name === "manifest.json") return mockFile(JSON.stringify(mockManifest))
          if (name === "sync-storage.json") return mockFile(JSON.stringify({ testKey: "testValue" }))
          if (name === "local-storage.json") return mockFile(JSON.stringify({ localKey: "localValue" }))
          if (name === "database.sqlite") return { async: vi.fn().mockResolvedValue(new Uint8Array([1])) }
          if (name === "chat-db.json") return { async: vi.fn().mockResolvedValue(new Blob()) }
          if (name === "vector-db.json") return { async: vi.fn().mockResolvedValue(new Blob()) }
          if (name === "knowledge-db.json") return { async: vi.fn().mockResolvedValue(new Blob()) }
          return null
        })
      }
      ...
      expect(chrome.storage.local.set).toHaveBeenCalledWith({ localKey: "localValue" })
