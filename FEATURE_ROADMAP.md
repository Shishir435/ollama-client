...
`windows` (Firefox sidepanel fallback), `commands` (global hotkeys, F2),
`notifications` + `alarms` (E5), `omnibox` (E6), Web Speech API
(`speechSynthesis`).
...
| API                        | Status | Unlocks                                                                                              |
| -------------------------- | ------ | ---------------------------------------------------------------------------------------------------- |
| `commands`                 | wired  | Global hotkeys (open panel, new chat, summarize tab) — F2, shipped `v0.11.1`                          |
| `tabs.captureVisibleTab`   | wired  | Screenshot → vision model on live pages — E1, shipped `v0.11.5`                                       |
| `alarms`                   | wired  | Durable scheduled jobs that survive SW suspension — E5, shipped `v0.11.6`                             |
| `notifications`            | wired  | "Long job done" (embed rebuild, backfill, big summary) — E5, shipped `v0.11.6`                        |
...
### E6. Omnibox Quick-Ask — 🟢 `v0.11.7` ✅ shipped

**What:** Type the keyword `olc ` in the address bar followed by a question; the
side panel opens and the question is sent as a chat turn, without opening the UI
first.
...
- Keyword is **`olc`** (reserved to match a future `olc` CLI), not `ol`.
- Always opens the **side panel** (the only chat surface). The address-bar
  `disposition` (Enter vs Alt/Meta+Enter) is forwarded but intentionally ignored.
- `sidePanel.open()` is called **synchronously** on `onInputEntered` from a
  pre-cached active tab, because any `await` first would consume the user gesture
  and Chrome would fall back to a popup window.
- The query is held in storage until a model has hydrated (avoids hanging at
  "Preparing context…"), de-duped across the storage + runtime-message delivery
  paths, and expired via a 60s TTL so a never-consumed query is not auto-sent on
  a much later open.
...
| ✅     | `v0.11.3`  | F1 — MV3 lifecycle hardening (timeouts ✅; alarms + resumable jobs follow-up)                                                                                     | A · foundation     | 🟢      | —               |
