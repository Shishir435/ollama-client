...
describe("chatSessionStore", () => {
  ...
  it("should set current session ID and trigger message load", async () => {
    ...
    const equalsMock = vi
      .fn()
      .mockReturnValue({ reverse: reverseMock, sortBy: sortByMock })
    const _whereMock = vi
      .mocked(db.messages.where)
      .mockReturnValue({ equals: equalsMock } as any)
  ...
  it("should delete session and its associated data", async () => {
    ...
    const reverseMock = vi.fn().mockReturnValue({ sortBy: sortByMock })
    const toArrayMock = vi.fn().mockResolvedValue([])
    const messagesEqualsMock = vi.fn().mockReturnValue({
      delete: deleteMock,
      reverse: reverseMock,
      sortBy: sortByMock
    })

    const filesEqualsMock = vi
      .fn()
      .mockReturnValue({ delete: deleteMock, toArray: toArrayMock })
  ...
  it("should handle embedding deletion failure gracefully", async () => {
    ...
    const equalsMock = vi.fn().mockReturnValue({
      delete: deleteMock,
      sortBy: sortByMock,
      reverse: vi.fn().mockReturnValue({ sortBy: sortByMock })
    })
    vi.mocked(db.messages.where).mockReturnValue({ equals: equalsMock } as any)
