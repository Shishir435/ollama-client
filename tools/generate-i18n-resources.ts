...
const SELECTION_OUTPUT_FILE = path.join(
  __dirname,
...
async function generateResources() {
  ...
  const selectionResources: Record<string, { translation: unknown }> = {}
  ...
  for (const locale of locales) {
    ...
    if (fs.existsSync(translationFile)) {
      try {
        ...
        selectionResources[locale] = {
          translation: {
            selection_button: json.selection_button
          }
        }
...
`
  ...
  const selectionFileContent = `// This file is auto-generated by tools/generate-i18n-resources.ts
// Do not edit this file directly

export const selectionResources = ${JSON.stringify(selectionResources, null, 2)} as const;
`
  fs.writeFileSync(SELECTION_OUTPUT_FILE, selectionFileContent)
  console.log(`✨ Generated selection resources at ${SELECTION_OUTPUT_FILE}`)
  ...
  try {
    ...
    execSync(
      `npx biome check --write --vcs-use-ignore-file=false "${OUTPUT_FILE}" "${SELECTION_OUTPUT_FILE}" "${CHROME_LOCALES_DIR}"`,
      { stdio: "inherit" }
    )
